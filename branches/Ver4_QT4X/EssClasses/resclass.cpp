...
RESClass::RESClass(IBPP::Database MyDB, long ID_IN)
		:Eidos(MyDB, ID_IN)
{
...
RESClass::RESClass(IBPP::Database MyDB)
		:Eidos(MyDB)
{
	//Перегруженный конструктор в качестве параметра получает идентификатор
        //объекта класса
...
RESType* RESClass::GetRESType(long RESID)
{
...
//на основании идентификатора паереданного в параметре
	RESType*TmpPtr= new RESType(this,RESID);
        return TmpPtr;
}

RESType* RESClass::AddRESType()
{
//Процедура создает объект типа OBJType принадлежащий к текущему классу объектов учета
//на основании идентификатора паереданного в параметре
	RESType*TmpPtr= new RESType(this,"Новый подучетный ресурс");
        return TmpPtr;
}

std::string RESClass::GetClassSQL()
{
	return "select * from GET_EIDOS_LIST('RES') where ID>1;";
}
