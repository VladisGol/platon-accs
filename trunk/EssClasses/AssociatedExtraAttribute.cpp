...
{
//Процедура запускает процедуру привязанную к экстраатрибуту и возвращает значение в виде структуры
	...
	TLocateOptions Opts;
        ...
        if(DM->MemTableEh_Plugins->Locate("DLL_name;Name", VarArrayOf(locvalues, 1),Opts))
        {
                ...
                if(DLLRoutine!=NULL)
                {
                	...
                	{
                		std::cerr << "Сбой вызова подгружаемой процедуры";
	                }
                }
                else
                {
                	std::cerr << "Для функции "+this->DLL_ProcName+" в библиотеке "+this->DLL_FileName+" отсутствует точка вхождения";
                }
        }Нужно переписать без применения модуля DM
}*/
